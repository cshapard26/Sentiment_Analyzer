...
public:
    ...
    std::vector<DSString> tokenize(const char) const;

    /**
     * @brief Searches for a DSString in a DSString vector.
     *
     * This is a `friend` function because it does not act directly on a DSString
     * but is only used to find strings in DSString vectors.
     * 
     * @return bool (true if the string was found)
     */
    friend bool DSFind(std::vector<DSString>&, DSString);

    /**
     * @brief Returns a new DSString object with all non-aplhabetic characters removed and in lowercase.
     *
     * @return DSString
     */
    DSString cleanString() const;

    /**
     * @brief Returns an integer representation of a DSString, 
     * or 0 if it is not a numeric value
     *
     * @return int
     */
    int DSToInt() const;
